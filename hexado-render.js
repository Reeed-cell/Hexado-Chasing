 ...
 *
 * Classes:
 *   TerrainGen     — smooth multi-octave displaced PlaneGeometry
 *                    Road corridor flattened at x ∈ [–9, +9]
...
/* ════════════════════════════════════════════════════════════
   ...
   Road corridor fix: within ±ROAD_HALF_W world-units of X=0
   the height is blended to a constant ROAD_H so every road
   segment sits at exactly the same Y.  No more stair-steps.
   ════════════════════════════════════════════════════════════ */
HE.TerrainGen = class {

  static ROAD_H      = 0.68;   // flat height under the road
  static ROAD_HALF_W = 9.5;    // blend radius around x = 0

  /**
   * Multi-octave sine noise height, with road-corridor flattening.
   */
  static heightAt(wx, wz) {
    const natural = Math.max(0,
      Math.sin(wx * 0.025 + 0.3) * Math.cos(wz * 0.020 + 0.8) * 2.8 +
      Math.sin(wx * 0.060 + wz  * 0.045 + 0.7) * 1.1 +
      Math.cos(wx * 0.130 - wz  * 0.100 + 1.2) * 0.5 +
      Math.sin(wx * 0.280 + wz  * 0.320)        * 0.18 +
      0.65
    );

    // Smooth blend toward ROAD_H inside the road corridor.
    // blend = 1 at x=0 (fully flat), 0 at |x| >= ROAD_HALF_W (fully natural).
    const t = Math.max(0, 1.0 - (Math.abs(wx) / this.ROAD_HALF_W) ** 2);
    return natural * (1 - t) + this.ROAD_H * t;
  }
  ...
  static generate(scene) {
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      let r, g, b;
      if      (h < 0.45) { r = 0.18; g = 0.30; b = 0.11; }
      else if (h < 1.30) { r = 0.22; g = 0.40; b = 0.12; }
      else if (h < 2.20) { r = 0.27; g = 0.44; b = 0.14; }
      else if (h < 3.10) { r = 0.40; g = 0.36; b = 0.17; }
      else               { r = 0.52; g = 0.45; b = 0.22; }
...
HE.EnvironmentGen = class {
  ...
  static build(scene) {
    ...
    this._buildFarm(scene, hAt, -36, -22, false);
    this._buildFarm(scene, hAt,  30,  28, true);
  ...
  /* ── Country road ──
     Because TerrainGen.heightAt now returns ROAD_H at x=0 for all z,
     every segment has identical Y → perfectly flat, seamless road.    */
  static _buildRoad(scene, hAt) {
    ...
    for (let z = -125; z < 125; z += SEG) {
      // All road heights are now identical because heightAt(0, *) = ROAD_H
      const h = hAt(0, z + SEG * 0.5) + 0.04;

      const sL = new THREE.Mesh(new THREE.BoxGeometry(1.4, 0.06, SEG + 0.1), shoulder);
      sL.position.set(-5.6, h, z + SEG * 0.5); scene.add(sL);
      const sR = new THREE.Mesh(new THREE.BoxGeometry(1.4, 0.06, SEG + 0.1), shoulder);
      sR.position.set( 5.6, h, z + SEG * 0.5); scene.add(sR);

      const road = new THREE.Mesh(new THREE.BoxGeometry(9.4, 0.07, SEG + 0.1), asphalt);
      road.position.set(0, h, z + SEG * 0.5); scene.add(road);

      const eL = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.02, SEG + 0.1), edgeLine);
      eL.position.set(-4.5, h + 0.01, z + SEG * 0.5); scene.add(eL);
      const eR = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.02, SEG + 0.1), edgeLine);
      eR.position.set( 4.5, h + 0.01, z + SEG * 0.5); scene.add(eR);
    }
  ...
  static _buildStormClouds(scene) {
    ...
    positions.forEach(([cx, cy, cz, mat]) => {
      ...
      this._m(new THREE.SphereGeometry(5.2, 5, 4), mat,   0,   -2.5, 0, g);
      scene.add(g);
    });
  }
};
...
HE.AssetFactory = class {
  ...
  static createVehicle() {
    ...
    const bGeo = new THREE.SphereGeometry(0.07, 5, 4);
    wPos.forEach(([x, y, z]) => {
...
HE.HUD = class {
  ...
  constructor() {
    ...
    this._elEnter   = document.getElementById('enter-prompt');
    this._elMode    = document.getElementById('mode-label');
    this._alertClr  = null;
   ...
   * @param {boolean}          canEnter    — show "Press E" prompt
   * @param {string}           stormState  — 'clear'|'forming'|'active'|'dissipating'
   */
  update(stats, tracker, playerPos, inVehicle, canEnter, stormState) {
    ...
    this._elProx.textContent  = isFinite(stats.proximity) && tracker.intensity > 0.02
      ? Math.round(stats.proximity) : '---';
    this._elScore.textContent = Math.round(stats.score);

    // EF bar — grey out when storm is clear
    const active = stormState !== 'clear' && tracker.intensity > 0.01;
    const pct = active ? Math.round(tracker.intensity * 100) : 0;
    this._elEfFill.style.width = pct + '%';
    const efColors = ['#00cc44', '#88cc00', '#ffcc00', '#ff8800', '#ff4400', '#cc0000'];
    this._elEfFill.style.background = active ? (efColors[tracker.efScale] || '#00cc44') : '#444444';

    // Storm state label
    const stateLabels = {
      clear:        'Clear skies',
      forming:      `Storm forming  ·  EF${tracker.efScale}`,
      active:       `EF${tracker.efScale}  ·  ${Math.round(tracker.windSpeed)} km/h`,
      dissipating:  'Storm dissipating'
    };
    this._elEfText.textContent = stateLabels[stormState] || '';

    // Mode label
    if (this._elMode) {
      this._elMode.textContent = inVehicle ? '🚗 DRIVING' : '🚶 ON FOOT';
    }

    // Enter/exit prompt
    if (this._elEnter) {
      this._elEnter.style.opacity = canEnter ? '1' : '0';
      this._elEnter.textContent   = inVehicle ? '[E]  Exit vehicle' : '[E]  Enter vehicle';
    }

    // Proximity danger flash — only when storm is active
    if (active) {
  ...
  _drawMinimap(playerPos, stormPath) {
    ...
    const lb = [['#ff3300', 'STORM'], ['#44ccff', 'YOU']];
